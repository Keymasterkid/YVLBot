...
module.exports = {
  name: 'setleave',
  description: 'Sets the leave channel for the server. Mention a channel or provide a channel name.',
  category: 'Moderation',
  async execute(message, args, client, prefix, db) {
    ...
    try {
      ...
      if (existingEntry) {
        // Update the existing leave channel
        await db.run('UPDATE servers SET leave_channel = ? WHERE server_id = ?', [channel.id, serverId]);
        message.reply(`Leave channel has been updated to <#${channel.id}>.`);
      } else {
        // Insert a new server entry with the leave channel
        await db.run('INSERT INTO servers (server_id, leave_channel) VALUES (?, ?)', [serverId, channel.id]);
        message.reply(`Leave channel has been set to <#${channel.id}>.`);
      }

      // Send a confirmation message in the leave channel
      await channel.send('This channel has been set as the Leave channel! 👋');

    } catch (error) {
      console.error('Error setting leave channel:', error);
      message.reply('There was an error trying to set the leave channel. Please try again later.');
    }
  },
};
